...
public partial class admin_systemset_areatownsetdetail : SystemPopUpBasePage
{
    ...
    private void GetPageElementValue()
    {
        if (Request.QueryString["curId"] != null)
        {
            ...
            UtilitySearch.LoadCounty(this.ddlCountyID, obj.CityID, Action, obj.CityID, "请选择");
            if (obj.IsPersistent)
            {
                ControlAndValue.GetValues4Control<areatownEntity>(this.form1, obj);
            }
    ...
    {
        #region 验证
        if (txtTitle.Text.Trim().Length == 0) { AlertError("乡镇名称不可以为空！"); Select(txtTitle); return; }
        ...
        ajaxEntity.Action = SystemData.CheckTownNameExist;
        ...
        string returnValue = sset.CheckTownNameExist(ajaxEntity);
        if (returnValue == "Y")
        {
            AlertError("乡镇名称已存在,请重新输入！");
        ...
        areatownEntity objEntity = new areatownEntity();
        ...
        ControlAndValue.SetValues4Controls<areatownEntity>(this.form1, objEntity);
    ...
    private void LoadScript()
    {
        ...
        sb.AppendFormat("<script type=\"text/javascript\" src=\"{0}\"></script>", CurrentDomainRootPath + "scripts/admin/systemset/areatownsetdetail.aspx.js?v=" + CommonMethod.Version);
