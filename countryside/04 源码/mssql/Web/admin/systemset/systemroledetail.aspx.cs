...
using System.Text;
using BLL.Service;
public partial class admin_systemset_systemroledetail : SystemPopUpBasePage
{
    ...
    private void GetPageElementValue()
    {
        if (Request.QueryString["curId"] != null)
        {
            ...
            if (obj.IsPersistent)
            {
                ControlAndValue.GetValues4Control<systemroleEntity>(this.form1, obj);
            }
    ...
    {
        #region 验证
        if (txtTitle.Text.Trim().Length == 0) { AlertError("角色名称不可以为空！"); Select(txtTitle); return; }
        ...
        ajaxEntity.Action = SystemData.CheckRoleNameExist;
        ajaxEntity.PageStatus = "2";
        ajaxEntity.AutoID=txtAutoID.Text.ToInt(0);
        SystemSet sset = new SystemSet(ajaxEntity.PageStatus);
        string returnValue = sset.CheckRoleNameExist(ajaxEntity);
        if (returnValue == "Y")
        {
             AlertError("角色名称已存在,请重新输入！");
            Select(txtTitle);  
            return;
        }
        #endregion
        ...
        ControlAndValue.SetValues4Controls<systemroleEntity>(this.form1, objEntity);
        objEntity.Save();
        setNull();
    ...
    private void LoadScript()
    {
        ...
        sb.AppendFormat("<script type=\"text/javascript\" src=\"{0}\"></script>", CurrentDomainRootPath + "scripts/admin/systemset/systemroledetail.aspx.js?v=" + CommonMethod.Version);
