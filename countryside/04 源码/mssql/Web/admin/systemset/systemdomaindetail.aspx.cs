...
public partial class admin_systemset_systemdomaindetail : SystemPopUpBasePage
{
    ...
    private void GetPageElementValue()
    {
        if (Request.QueryString["curId"] != null)
        {
            ...
            if (obj.IsPersistent)
            {
                ControlAndValue.GetValues4Control<systemdomainEntity>(this.form1, obj);
    ...
    {
        #region 验证
        if (txtTitle.Text.Trim().Length == 0) { AlertError("用户名不可以为空！"); Select(txtTitle); return; }
        ...
        systemdomainEntity objEntity = new systemdomainEntity();
        ...
        ControlAndValue.SetValues4Controls<systemdomainEntity>(this.form1, objEntity);
         objEntity.Save();
    ...
    private void LoadScript()
    {
        ...
        sb.AppendFormat("<script type=\"text/javascript\" src=\"{0}\"></script>", CurrentDomainRootPath + "scripts/admin/systemset/systemdomaindetail.aspx.js?v=" + CommonMethod.Version);
